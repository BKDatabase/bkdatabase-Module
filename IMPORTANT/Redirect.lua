...
function p.getTarget(page, fulltext)
	...
	elseif type(page) == 'table' and type(page.getContent) == 'function' then
		...
		error(string.format(
			"đối số hỏng #1 cho 'getTarget'"
				.. " (chuỗi, số, hoặc đối tượng tiêu đề được mong đợi, đã lấy %s)",
			type(page)
		), 2)
	end
	...
	if target then
		...
		error(string.format(
			'không thể phân tích trang đổi hướng "%s"',
...
function p.main(frame)
	local args = require('Mô đun:Arguments').getArgs(frame, {frameOnly = true})
