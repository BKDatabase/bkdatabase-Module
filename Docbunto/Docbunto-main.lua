...
local i18n = require('Module:I18n/Special').loadMessages('Docbunto'):useContentLang()
...
local syntaxhighlightPresent = mw.getCurrentFrame()
	:preprocess("<syntaxhighlight lang=\"lua\">-- Lua</syntaxhighlight>")
		~= "<syntaxhighlight lang=\"lua\">-- Lua</syntaxhighlight>"
...
--  @field[opt]         {boolean} options.noluaref Don't link to the [[mw:
--  					Extension:Scribunto/Lua reference manual|Lua reference manual]] for types.
...
function frontend.type_reference(item, options)
	...
	for index, item_type in ipairs(item.types) do
		item.types[index] = item.types[index]:gsub('[%w_. ]+', function(item_type)
			...
			elseif
				item_type:find(i18n:msg('line-no'):gsub('$1', ''))
			then
	...
	if item.value then
		...
		if item.value:find('^{{.+}}$') then
			...
			if item.value:match('^{{([^:]+)') == '#invoke' then
				item.value = item.value:gsub('^{{#invoke:', '{{mlx' .. multi_line)

			else
				if options.entrypoint then
					item.value = item.value:gsub('^([^|]+)|%s*([^|}]-)(%s*)([|}])','%1|"%2"%3%4')
				end
				item.value = item.value:gsub('^{{', '{{tlx' .. multi_line)
			end
			...
			if item.value:find('\n') then
				local highlight_div = mw.html.create('div')
			...
			local highlight_tag = mw.html.create('syntaxhighlight')
...
function frontend.pretask_error_line(item, options)
	if item.name then
		...
		if line then
			if item.types then
				item.types[#item.types + 1] = i18n:msg('line-no', line)

			else
				item.type = i18n:msg('line-no', line)
			end
		end
...
function frontend.render_item(stream, item, options, pretask)
	...
	-- add single spaces between parentheses similar to
	-- MediaWiki documentation
	if not item_name:find( '()', 1, true ) then
	...
	if not usingOptionsContent then
		...
		if fileTooBig or not syntaxhighlightPresent then
			span_node:wikitext('[{{fullurl:' .. options.file .. '|action=edit#mw-ce-l' .. item.lineno .. '}} ' .. tostring(code_node) .. ']')
		elseif title.namespace == 828 and options.file == title.text then
			span_node:wikitext('[[' .. '#L-' .. item.lineno .. '|' .. tostring(code_node) .. ']]')
		else
			span_node:wikitext('[[' .. options.file .. '#L-' .. item.lineno .. '|' ..  tostring(code_node) .. ']]')
		end
	else
...
--- Template entrypoint for [[Template:Lua autodoc]].
--  @function           p._main
...
function p._main(args)
	...
	options.all = yesno(args.all or false)
	options.autodoc = yesno(args.autodoc or false)
	options.boilerplate = yesno(args.boilerplate or options.autodoc or false)
	options.box = yesno(args.box or args.infobox or options.autodoc or false)
	options.caption = args.caption
	options.card = yesno(args.card or (title.namespace == 828 and not options.autodoc))
	options.code = yesno(args.code or false)
	options.colon = yesno(args.colon or false)
	options.footer = yesno(args.footer or title.namespace == 828)
	options.image = args.image
	options.migration = yesno(args.migration or false)
	options.noluaref = yesno(args.noluaref or false)
	options.plain = yesno(args.plain or false)
	options.preface = args.preface
	options.simple = yesno(args.simple or false)
	options.sort = yesno(args.sort or false)
	options.strip = yesno(args.strip or false)
	options.verbose = yesno(args.verbose or false)
	options.ulist = yesno(args.ulist or false)
...
--  @function		   p.main
--  @param			  {Frame} frame Module frame.
--  @return			 {string} Module documentation output.
...
function p.taglet(modname, options)
	...
	if not p_success then
		mw.addWarning("Docbunto error: " .. p_error)
	end
...
function p.doclet(data, options)
	...
	-- Information
	if not options.code and options.box then
