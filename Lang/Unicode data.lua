...
local function get_codepoint(args, arg)
	local codepoint_string = args[arg]
		or errorf(2, "Tham số %s là bắt buộc", tostring(arg))
	local codepoint = tonumber(codepoint_string, 16)
		or errorf(2, "Tham số %s không phải là một điểm mã trong cơ sở thập lục phân",
			tostring(arg))
	if not (0 <= codepoint and codepoint <= 0x10FFFF) then
		errorf(2, "điểm mã trong tham số %s ngoài phạm vi", tostring(arg))
	end
	return codepoint
end
...
local function get_func(args, arg, prefix)
	...
	local func = p[func_name]
		or errorf(2, "Không có hàm '%s'", func_name)
	return func
end
