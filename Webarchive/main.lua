...
--[[--------------------------< R E P L A C E _ V I E T N A M E S E _ D A T E >--------------------------------------------------------

This function converts a Vietnamese date string into an English format, specifically handling structures
like "ngày 1 tháng 8 năm 2009" or "1 tháng 8 năm 2009" and transforming them into "1 August 2009." This
is accomplished by replacing Vietnamese date phrases with their English counterparts. It is especially
helpful for the decode_date() function, which determines whether the date provided matches archived
records from sources such as archive.today or the Wayback Machine.
...
local function decode_date (date_str)
	date_str = replace_vietnamese_date(date_str)								-- convert the input Vietnamese date from date_str before decoding, if it exists
	...
	for k, v in pairs (patterns) do
		...
		if c1 then																-- set on match
			...
			if month_num[t.m] then												-- when month not already a number
				...
				return nil, 'dmy';												-- not a valid date form because month not valid
			end

			return mw.ustring.format ('%.4d-%.2d-%.2d', t.y, t.m, t.d), k;		-- return date in iso format
		end
	end
	return nil, 'dmy';															-- date could not be decoded; return nil and default iso date
end
...
local function createRendering()
	...
	ulx.url1.date = ulx.url1.date:gsub ('index.*', 'lưu trữ');					-- remove message
	...
	if 'none' == ulx.url1.format then											-- For {{wayback}}, {{webcite}}
		...
		if ulx.url1.title then
			...
			if ulx.url1.date then
				...
				table.insert (out, 'lưu trữ' == ulx.url1.date and s_text.archive or s_text.archived);	-- add text
			...
			if index_date then													-- when url date is 'index' 
				...
				table.insert (out, table.concat ({' ', s_text.Archived, '] ngày '}));	-- add link label for url has timestamp date (will include mismatch message if there is one)
			end
			if ulx.url1.date then
				if 'lưu trữ' ~= ulx.url1.date then
...
local function webarchive(frame)
	...
	local ldf = 'dmy';															-- when there is no |date= parameter, render url dates in iso format
...
--[[																				-- at Template:Webarchive/testcases/Production; resolve that before deleting these tests
	...
	if 'wayback' == ulx.url1.service or 'locwebarchives' == ulx.url1.service or 'archiveit' == ulx.url1.service or 'ukgwa' == ulx.url1.service then
		if date then
