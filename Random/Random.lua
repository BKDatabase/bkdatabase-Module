...
local makeList = require('Mô đun:List').makeList
...
local function raiseError(msg)
	...
	return mw.ustring.format('<b class="error">Lỗi [[Mô đun:Random]]: %s.</b>', msg)
end
...
function l.date(args)
	...
	if not startTimestampUnix then
		return raiseError('"' .. tostring(startTimestamp) .. '" không được công nhận là dấu thời gian hợp lệ')
	elseif not endTimestampUnix then
		return raiseError('"' .. tostring(endTimestamp) .. '" không được công nhận là dấu thời gian hợp lệ')
	elseif startTimestampUnix > endTimestampUnix then
		return raiseError('ngày bắt đầu phải không được muộn hơn ngày kết thúc (ngày bắt đầu: "' .. startTimestamp .. '", ngày kết thúc: "' .. endTimestamp .. '")')
	end
	...
	local dateFormat = args.format or 'H:i, "ngày" d F "năm" Y (T)'
	local result = getDate(dateFormat, '@' .. tostring(randomTimestamp))
	if result then
		return result
	else
		return raiseError('"' .. dateFormat .. '" không phải là một định dạng ngày hợp lệ')
	end
end
...
function l.html_list(args, listType)
	-- Randomizes a list and turns it into an HTML list. Uses [[Mô đun:List]].
	listType = listType or 'bulleted'
	local listArgs = makeRandomList(args) -- Arguments for [[Mô đun:List]].
...
function p.main(funcName, args, listType)
	...
	if type(args) ~= 'table' then
		error('đối số thứ hai với p.main phải là một bảng')
	end
