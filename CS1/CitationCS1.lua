...
local function vi_formatdate(rawDate, dayPrefix)
	...
	rawDate = mw.ustring.gsub(rawDate, "Tháng", "tháng", 1)
	if dayPrefix then formatStr = '"ngày" ' .. formatStr end
	
	-- Khoảng năm "c." hoặc "k."
	if mw.ustring.match(rawDate, "^%s*[ck]%.%s*%d%d%d%d?$") then
		rawDate = mw.ustring.gsub(rawDate, "c.", "k.", 1)
	...
	-- Ngày tháng năm
	elseif mw.ustring.match(rawDate, "^[Nn]gày %d%d? tháng %d%d? năm %d%d%d%d$") then
		local day, month, year = mw.ustring.match(rawDate, "^[Nn]gày (%d%d?) tháng (%d%d?) năm (%d%d%d%d)$")
			...
			if one then															-- if <one> has a value (name, mdash replacement, or mask text replacement)
				...
				if tag then
					...
					if lang then												-- error messaging done in extract_names() where we know parameter names
						lang = mw.ustring.gsub(lang, "^[Tt]iếng ", "", 1);
...
local function citation0( config, args )
	...
	if not utilities.is_set (Date) then
		...
		if not utilities.is_set (Date) and utilities.is_set (PublicationDate) then	-- use PublicationDate when |date= and |year= are not set
			...
			Date = vi_formatdate(PublicationDate, true);
			PublicationDate = '';												-- unset, no longer needed
			Date_origin = A:ORIGIN('PublicationDate');							-- save the name of the promoted parameter
		else
			Date_origin = A:ORIGIN('Year');										-- save the name of the promoted parameter
		end
		if PublicationDate and #PublicationDate > 0 then PublicationDate = vi_formatdate(PublicationDate, true) end
	else
		Date_origin = A:ORIGIN('Date');											-- not a promotion; name required for error messaging
	end

	if PublicationDate == Date then PublicationDate = ''; end					-- if PublicationDate is same as Date, don't display in rendered citation
	if Date and #Date > 0 then Date = vi_formatdate(Date, true) end
...
--					utilities.set_message ('maint_unfit');						-- and add a category if not already added
	...
	if utilities.is_set (PublicationDate) then
		PublicationDate = wrap_msg ('published', vi_formatdate(PublicationDate, true));
	end
	...
	OrigDate = utilities.is_set (OrigDate) and wrap_msg ('origdate', vi_formatdate(OrigDate, true)) or '';
	...
	local template_name = ('citation' == config.CitationClass) and 'Chú thích' or 'Chú thích ' .. (cfg.citation_class_map_t[config.CitationClass] or config.CitationClass);
	local template_link = '[[Bản mẫu:' .. template_name .. '|' .. template_name .. ']]';
