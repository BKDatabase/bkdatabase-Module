...
function chessboard(args, size, rows, cols, rev, trans, lightdark, altprefix, letters, numbers, header, footer, align, clear)
    ...
    function letters_row( rev, num_lt, num_rt, cols )
        local res = '<tr style="vertical-align:middle">' .. ( num_lt and '<th style="padding:0; vertical-align:inherit; font-weight:normal"></th>' or '' ) .. '<th style="padding:0; vertical-align:inherit; height:18px; font-weight:normal">'
        for k = 1, cols do
            res = res .. colchar(rev and (cols - k + 1) or k) .. '</th><th style="padding:0; vertical-align:inherit; font-weight:normal">'
        end
        res = res .. '</th>' .. ( num_lt and '<th style="padding:0; vertical-align:inherit; font-weight:normal"></th>' or '' ) .. '</tr>'
        return res
    end
    ...
    for trow = 1,rows do
        ...
        if ( numbers_lt ) then b = b .. '<th style="padding:0; vertical-align:inherit; width:18px; font-weight:normal">' .. row .. '</th>' end
        for tcol = 1,cols do
        ...
        if ( numbers_rt ) then b = b .. '<th style="padding:0; vertical-align:inherit; width:18px; font-weight:normal">' .. row .. '</th>' end
        b = b .. '</tr>'
    end
